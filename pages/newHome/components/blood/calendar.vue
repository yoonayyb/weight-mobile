<template>
	<view class="u-calendar">
          ...
          'u-calendar__content--end-date':
            ...
            color: getColor(index, 2)
              ...
              lineHeight: '56rpx'
				...
				<view v-if="naviTitle !== 'suggest'"
					style="width: 56rpx;height: 28rpx;display: flex;justify-content: center;"
					>
					 <text style="width: 10rpx;height: 10rpx;border-radius: 50%;margin-top: 10rpx;" :style="{background:setColor(year, month, index + 1)}" ></text>
					</view>
		...
		<view class="u-calendar__bottom">
			<block v-if="naviTitle === 'blood'">
				<view class="u-calendar__bottom__custom u-m-r-8"><text
				style="width: 20rpx;height: 20rpx;border-radius: 50%;margin-right: 8rpx;"
						:style="{ background: calendarData.color.COLOR_NORMAL }"></text> 血糖正常</view>
				<view class="u-calendar__bottom__custom u-m-r-8"><text
				style="width: 20rpx;height: 20rpx;border-radius: 50%;margin-right: 8rpx;"
						:style="{ background: calendarData.color.COLOR_ABNORMAL }"></text>血糖异常</view>
			</block>
			<block v-if="naviTitle === 'fast'">
				<view class="u-calendar__bottom__custom u-m-r-8"><text
				style="width: 20rpx;height: 20rpx;border-radius: 50%;margin-right: 8rpx;"
						:style="{ background: calendarData.color.COLOR_NORMAL }"></text> 已达标</view>
				<view class="u-calendar__bottom__custom u-m-r-8"><text
				style="width: 20rpx;height: 20rpx;border-radius: 50%;margin-right: 8rpx;"
						:style="{ background: calendarData.color.COLOR_ABNORMAL }"></text>未达标</view>
			</block>
			<block v-if="naviTitle === 'blood-pressure'">
				<view class="u-calendar__bottom__custom u-m-r-8"><text
				style="width: 20rpx;height: 20rpx;border-radius: 50%;margin-right: 8rpx;"
						:style="{ background: calendarData.color.COLOR_NORMAL }"></text> 血压正常</view>
				<view class="u-calendar__bottom__custom u-m-r-8"><text
				style="width: 20rpx;height: 20rpx;border-radius: 50%;margin-right: 8rpx;"
						:style="{ background: calendarData.color.COLOR_ABNORMAL }"></text>血压异常</view>
			</block>
			<block v-if="naviTitle === 'suggest'">
				<view class="u-calendar__bottom__custom u-m-r-8"><text
				style="width: 20rpx;height: 20rpx;border-radius: 50%;margin-right: 8rpx;"
						:style="{ background: '#00B8A2' }"></text> 我的感受</view>
				<view class="u-calendar__bottom__custom u-m-r-8"><text
				style="width: 20rpx;height: 20rpx;border-radius: 50%;margin-right: 8rpx;"
						:style="{ background: '#3E77E9' }"></text>营养师评价</view>
			</block>
	...
	export default {
		...
		data() {
			return {
				...
				calendarData: {
					color: {
						COLOR_ABNORMAL: '#FFC76B',
						COLOR_NORMAL: '#00B8A2'
					},
					data: {
						// '2022-09-08': 1, //日期=>状态 (1:正常,0异常)
						// '2022-09-07': 1,
						// '2022-09-06': 0,
						// '2022-09-05': 1
					}
		...
		computed: {
			...
			setColor() {
				return function(year, month, day) {
					...
					return this.calendarData.data[_date] === 1 ?
						this.calendarData.color.COLOR_NORMAL :
						this.calendarData.data[_date] === 0 ?
						this.calendarData.color.COLOR_ABNORMAL :
						'white'
				}
			},
			setColor1() {
		...
		methods: {
			...
			getCalendarData(date) {
				...
				let url;
				let data;
				if (this.naviTitle === 'blood') {
					url = '/ApiV2/Mobile/Diary/bloodSugarDate'
					data = {
						date: date + ' 00:00:00',
						user_id: this.userId ? this.userId : 0
					}
				} else if (this.naviTitle === 'blood-pressure') {
					url = '/ApiV3/Mobile/Diary/bloodPressreDate'
					data = {
						date: date + ' 00:00:00',
						user_id: this.userId ? this.userId : 0
					}
				}
				else if (this.naviTitle === 'suggest') {
					url = '/ApiV3/Mobile/Diary/otherDate'
					data = {
				...
				this.$request({
					...
					console.log('当前日历', res)
					if(this.naviTitle === 'suggest'){
						this.suggestArr = res.data.chart
