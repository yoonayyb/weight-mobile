	...
	<view class="u-switch" :class="[value == true ? 'u-switch--on' : '', disabled ? 'u-switch--disabled' : '']" @tap="onClick"
	 :style="[switchStyle]" >
		<view class="u-switch__node node-class" :style="{
			width: $u.addUnit(this.size+12),
			height: $u.addUnit(this.size),
			background:'#00B8A2'
		}">
		<!-- <view class="u-font-20">{{value?'kcal':'kj'}}</view> -->
			<u-loading :show="loading" class="u-switch__loading" :size="size * 0.6" :color="loadingColor" />
		</view>
		<view class="u-font-20 u-flex u-flex-around u-relative" style="z-index: 99;" :style="[uniStyle]" >
			<view style="margin-left: 10rpx;" :style="{'color':value?'#4B4B4B':'#fff'}">kj</view>
			<view style="margin-left: 4rpx;" :style="{'color':value?'#fff':'#4B4B4B'}">kcal</view>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @property {String} active-color 打开时的背景色（默认#ffffff）
	...
	export default {
		...
		computed: {
			...
			uniStyle(){
				let style = {};
				style.lineHeight = uni.upx2px(this.size)  + 'px';
