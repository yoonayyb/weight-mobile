<template>
	<view class="container u-skeleton" @click="login">
		<block v-if="token">
			<u-navbar id="navbar" :border-bottom="false" :is-back="false" back-text="" title-width="750">
			...
			<u-mask :show="searchHospitalShow" @click="searchHospitalShow = false" z-index="80">
...
<script>
	...
	export default {
		data() {
			return {
				reflesh:false,
				scrollViewHeight:uni.getSystemInfoSync().windowHeight,
				messageCount: 0, //消息未读
				kefuId: '', //客服消息id
		...
		async onLoad(option) {
			this.scrollViewHeight = uni.getSystemInfoSync().windowHeight - uni.getSystemInfoSync().safeAreaInsets.bottom - uni.upx2px(350)
				...
				success(v) {
					...
					setTimeout(() => {
						query.select('#navbar').boundingClientRect(res => {
							...
							that.navigateTop = res.height 
							console.log('导航栏组件数据', res)
							console.log('this.navigateTop', that.navigateTop)
						}).exec()
					}, 500)
		...
		methods: {
			...
			gotoSearch() {
				uni.navigateTo({
					url: `/pages/home/shoppingMall/shopSearch?keyword=${this.keyword}`
				})
			},
			setKeyWord(val) {
				this.keyword = val
