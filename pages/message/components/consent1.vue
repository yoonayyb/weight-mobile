<template>
	<view v-if="copyItem.body && copyItem.body.type === 'ASSESSMENT_REPORT_FORM'" class="message_share_assess2 triangle" @tap="clickMessage">
		<view class="u-flex">
			<view class="view1 u-flex">
				<text class="share_assess_title2">{{copyItem.body.text}}</text>
				<text class="u-font-20" style="color:#888888">{{copyItem.body.time_limit}}</text>
				<view class="view2 u-flex u-m-t-24">
					
					<text  class="u-font-18 u-line-2"
	...
	export default {
		...
		watch: {
			item: {
				async handler(newValue, oldValue) {
					...
					if(this.copyItem.body.type !== 'ASSESSMENT_REPORT_FORM') return
					try {
					...
					if (this.copyItem.body.created_at) {
						let now = new Date().getTime()
						let countDownTime = new Date(this.copyItem.body.created_at).getTime()
		...
		methods: {
			...
			countDown(item) {
				...
				let now = new Date().getTime()
				let countDownTime = new Date(item.created_at).getTime()
			...
			clickMessage() {
				...
				//打开预览页面
				uni.navigateTo({
					url: '/pages/question/question3?id=' + id
				})
			}
		}
	}
