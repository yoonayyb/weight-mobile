      ...
      <input-popup2 btn="发送" title="请选择要发送的问卷" @confirm="handleContirm">
        <view class="ketonuria_content">
          ...
          >
            ...
            <text v-if="item.type == 2 && !hasQuestion" :style="item.checked ? '' : 'color: #00B8A2'">(未)</text>
          </view>
        </view>
      </input-popup2>
    </u-popup>
...
export default {
  ...
  methods: {
    ...
    handleContirm() {
	  ...
	  if(type == 1){
		  this.$request({
		    url: '/ApiV3/Mobile/AssessmentReport/v2send',
		    errMsg: true,
		    loading: true,
		    data: {
		      uid: this.friendUid
