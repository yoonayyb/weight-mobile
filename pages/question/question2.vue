<template>
	<view class="container">
		...
		<view class="box" v-if="tabNum == 1">
			...
			<view v-if="other" class="u-flex-col">
			...
			<view class="u-flex-col">
				...
				<u-input :height="88" v-model="formData.height" @input="numberFixedDigit($event,'height')" type="digit" input-align="left"
					placeholder-style="font-size:24rpx;" :custom-style="cs" placeholder="请填写" />
			</view>
			<view class="u-flex-col">
				<view style="font-size: 28rpx;font-weight: bold;">体重(kg)</view>
				<u-input :height="88" v-model="formData.weight" @input="numberFixedDigit($event,'weight')"  type="digit" input-align="left"
					placeholder-style="font-size:24rpx;" :custom-style="cs" placeholder="请填写" />
			</view>
		...
		<view v-if="tabNum == 2" class="form_question">
			...
			<u-form :label-style="{fontSize: '36rpx',fontWeight:'bold'}" :model="form" ref="uForm" label-position="top"
				...
				<u-form-item prop="health" label="1.您是否存在以下健康相关症状？">
					<u-checkbox-group active-color="#00B8A2">
						<u-checkbox @change="checkboxGroupChange" v-model="item.checked"
							v-for="(item, index) in healthList" :key="index" :name="item.value">
							{{ item.label }}
						</u-checkbox>
					</u-checkbox-group>
				</u-form-item>
				<u-form-item prop="drug" label="2.您当前在服用或注射的药物有？">
					<u-checkbox-group wrap active-color="#00B8A2" >
						<u-checkbox @change="checkboxGroupChange1" v-model="item.checked" v-for="(item, index) in drugList" :key="index"
							:name="item.value">
							{{ item.label }}
						</u-checkbox>
					</u-checkbox-group>
				</u-form-item>
				<u-form-item prop="NCD" label="3.您的直系亲属有以下慢性疾病史吗？">
					<view style="font-size: 28rpx;margin-bottom: 24rpx;line-height: 1.5;color:#D3D3D3">（包括父母亲、兄弟/姐妹、子女，祖父母和外组父母）</view>
					<u-checkbox-group wrap active-color="#00B8A2" >
						<u-checkbox @change="checkboxGroupChange2" v-model="item.checked" v-for="(item, index) in NCDList" :key="index"
							:name="item.value">
							{{ item.label }}
						</u-checkbox>
					</u-checkbox-group>
		...
		<view v-if="tabNum == 3">
			<Condition2  @gotohome="backHome = true" :formData="formData" :condition="condition1" :detailId="id" @submit="submit"></Condition2>
		...
		<view v-if="tabNum == 4" class="form_question">
			...
			<template v-else>
				...
				<template v-if="resultAll.result">
					...
					<view class="result" v-for="(item,index) of  resultAll.result.result1.slice(1)" :key="index">
...
<script>
	...
	import Condition2 from './components/condition2.vue'
	export default {
		data() {
			return {
				condition1:{},
				...
				healthList: [{
					...
					{
						label: '运动能力下降、乏力、易困嗜睡、打鼾、轻度脂肪肝、内分泌紊乱、血压正常高值、血脂边缘升高、糖尿病前期、高尿酸血症、黑色棘皮症等',
						checked: false,
						disabled: false,
						value: 2
					},
					{
						label: '高血压、高血脂、2型糖尿病、冠心病、痛风、多囊卵巢综合征、中重度脂肪肝、胆囊疾病、关节炎、冠脉支架术后、睡眠呼吸暂停综合征、女性不孕、男性性腺功能低下',
						checked: false,
						disabled: false,
						value: 3
					},
					{
						label: '心肌梗死(心脏病发作)、心脏衰竭、动脉粥样硬化、脑卒中、脑血栓、肝硬化、糖尿病并发症(如眼底疾病、肾衰竭等)、肥胖相关性癌症、肾功能不全、有精神疾病/中、重度抑郁症者、胰腺炎等',
						checked: false,
						disabled: false,
						value: 4
					},
		...
		watch: {
			'formData': {
				handler(val) {
					...
					let arr = this.other ? Object.keys(this.formData) : Object.keys(this.formData).filter(v => v !==
						'mobile')
		...
		methods: {
			...
			checkboxGroupChange(e) {
				...
				if (name === 1) {
					//这个是选择无 取消所有
					this.healthList.forEach(v => v.checked = value && v.value === name)
				} else {
					this.healthList.forEach(v => {
						if (v.value == 1) {
							v.checked = false
						}
						if(v.value == name){
				...
				this.form.health = this.healthList.filter(v => v.checked).map(v => v.value)
			...
			checkboxGroupChange2(e) {
				...
				if (name === 7) {
					//这个是选择无 取消所有
					this.NCDList.forEach(v => v.checked = value && v.value === name)
				} else {
					this.NCDList.forEach(v => {
						if (v.value == 7) {
							v.checked = false
						}
						if(v.value == name){
				...
				this.form.NCD = this.NCDList.filter(v => v.checked).map(v => v.value)
				
				
				
				// this.NCDList.forEach(v => {
				// 	if (e.includes(7) && v.value < 7) {
				// 		v.checked = false
				...
				// })
				// this.form.NCD = e
				console.log('this.form.NCD: ', this.form.NCD)
			},
			//多次解码
			getUrl(url) {
				if (url.indexOf('%') < 0) {
			...
			async submit(form1) {
				...
				if (this.tabNum === 1) {
					...
					if (this.other && !this.$u.test.mobile(this.formData.mobile)) return this.$common.toast('请输入正确手机号')
