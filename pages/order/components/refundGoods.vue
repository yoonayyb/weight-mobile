<template>
  <view class="goods_item_order_template"
    ...
    <u-checkbox-group>
      <u-checkbox :disabled="item.max == 0" class="uCheckbox" active-color="#00B8A2" inactive-color="#C2C2C2" shape="circle" size="40" v-for="(item, index) in cartList"
        :key="index" :name="item.id" v-model="item.checked" @change="checkboxChange(item,index)">
        <view class="checkbox">
          <image  @click.stop="jumpGoodsDetails(item)" style="border-radius: 16rpx;" class="goods_item_order_image" :src="item.goods_img" mode="aspectFill"></image>
            ...
            <view class="goods_item_order_bottom">
              ...
              <view class="goods_item_order_number" style="color:#C2C2C2">
                ...
                <u-number-box class="number_box" :disabled-input="true" bg-color="#00B8A2" color="#FFFFFF"
                  input-width="56" size="28" :min="0" :max="item.max" v-model="item.goods_number"
