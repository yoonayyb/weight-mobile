					...
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
					 :style="[tabItemStyle(index)]">
						...
						<view class="u-flex u-flex-column">
							{{ item[name] || item['name']}}
						<view v-if="showDot && currentIndex !== index" style="width: 12rpx;height: 12rpx;position: absolute;bottom: 0;border-radius: 50%;}" :style="{background: item.activeColor}">	
							.
						</view>
						</view>
					</view>
	...
	export default {
		...
		data() {
			return {
				...
				barFirstTimeMove: false, // 滑块第一次移动时(页面刚生成时)，无需动画，否则给人怪异的感觉
			};
		},
		watch: {
