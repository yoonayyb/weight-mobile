		...
		>
			...
			>
				<view
					...
					v-if="mode == 'number'"
					:style="{
						backgroundColor: current === index ?activeColor :current >index ?activedColor: 'transparent' ,
						borderColor:  current === index ?  activeColor :current > index ?activedColor: unActiveColor
					}"
				>
					<text v-if="current < index" :style="{
						color: current === index ?  activeColor :current > index ?activedColor: unActiveColor ,
					}">
						{{ index + 1 }}
					</text>
				...
				<view class="u-steps__item__dot" v-if="mode == 'dot'" :style="{ 
					backgroundColor: current === index  ? activeColor :current > index ?activedColor : unActiveColor,border:'1px solid' + (current >= index  ? activeColor  : unActiveColor)
				}"></view>
				<text class="u-line-1" :style="{ 
					color: current === index ? activeColor :current > index ?'#2A2A2A' : unActiveColor,
				...
				<view class="u-steps__item__line" :class="['u-steps__item__line--' + mode]" v-if="index < list.length - 1">
					<u-line :direction="direction" length="100%" :hair-line="false" :color="index <= current-1 ? activeColor : unActiveColor"></u-line>
				</view>
			</view>
		</view>
	</view>
