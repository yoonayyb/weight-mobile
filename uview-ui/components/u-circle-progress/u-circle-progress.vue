...
export default {
	...
	methods: {
		drawProgressBg() {
			...
			let ctx = uni.createSelectorQuery().in(this)
			ctx.select('#'+this.elBgId).fields({ node: true, size: true }).exec((res)=>{
		...
		async drawCircleByProgress(progress) {
			...
			if (!ctx) {
				...
				//修改
			    ctx = uni.createSelectorQuery().in(this)
				ctx.select('#'+this.elId).fields({ node: true, size: true }).exec((res)=>{
					 // Canvas 对象
					 const canvas = res[0].node
					 // Canvas 画布的实际绘制宽高
					 const renderWidth = res[0].width
