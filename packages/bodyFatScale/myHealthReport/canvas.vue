<template>
	<view class="u-circle-progress" :style="{
		...
		<canvas class="u-canvas" :canvas-id="elId" :id="elId" :style="{
				width: widthPx + 2* pointHeightPx+'px',
				height: widthPx/2 +4 +pointHeightPx+'px'
			}"></canvas>
		<slot></slot>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				...
				pointHeightPx:uni.upx2px(this.pointHeight),//转成px后的指针的高度
		...
		methods: {
			drawProgressBg() {
				let point = this.imgUrl + 'green_point.png'
				let that = this
				uni.getImageInfo({
					src: point,
					success(res) {
						that.path = res.path
						that.drawCircleByProgress(that.oldPercent);
					}
				})
			...
			drawCircleByProgress(progress) {
				...
				const cgrd = ctx.createLinearGradient(0, 0, this.widthPx, 0)
				cgrd.addColorStop(0, '#BBECE6')
				cgrd.addColorStop(progress / 100, this.activeColor)
				...
				// 结束角的计算依据为：将π分为100份，乘以当前的进度值，得出终止点的弧度值，加起始角，为整个圆从默认的
				// 3点钟方向开始画图，转为更好理解的12点钟方向开始作图，这需要起始角和终止角同时加上this.startAngle值
				let endAngle = ((1 * Math.PI) / 100) * progress + this.startAngle;
				...
				// 半径为整个canvas宽度的一半
				let radius = this.widthPx / 2;
				ctx.arc(radius+ uni.upx2px(40), radius + uni.upx2px(40), radius - this.borderWidthPx, this.startAngle + 1.5 * Math.PI, endAngle + 1.5 * Math
					.PI, false);
				ctx.stroke();
				...
				// 		let path = res.path
				// 		console.log('path', path)
				// 		ctx.drawImage(path, 0, 0, uni.upx2px(20), uni.upx2px(40))
				// 		ctx.draw();
				// 	}
				// })
				...
				if (this.newPercent > this.oldPercent) {
					...
					if (progress > this.newPercent){
						if(this.path && this.percent > 0){
								// ctx.translate(radius - this.borderWidthPx,radius - this.borderWidthPx) 
								ctx.translate(radius+uni.upx2px(40),radius+uni.upx2px(40))
								// ctx.translate()
								ctx.rotate(endAngle)
								ctx.translate(-uni.upx2px(10), -radius-uni.upx2px(18))
								
								console.log('endAngle + 1.5 * Math.PI',endAngle + 1.5 * Math.PI)
								ctx.drawImage(this.path, 0, 0, uni.upx2px(20), uni.upx2px(40))
