<template>
  <view class="number_keyboard " v-if="show" @touchmove.stop.prevent="clear" :style="{ zIndex: zIndex }">
    <view class="mast fade_in" @click="close" :class="{ fade_out: fadeOut }" :style="{ zIndex: zIndex + 1 }"> </view>
    <view
      class="number_keyboard_container bottom"
      :style="{ zIndex: zIndex + 2, bottom: bottom + 'px' }"
      :class="{ bottom_reverse: fadeOut }"
    >
      <slot />
      <view class="number_keyboard_header u-flex">
        <view class="number_keyboard_close" @click="close">
          <text>取消</text>
        </view>
        <view class="number_keyboard_title u-flex">
          <text>{{ title }}</text>
        </view>
        <view class="number_keyboard_btn u-flex" @click="handleClickRightBtn">
          <text v-if="rightBtnText">{{ rightBtnText }}</text>
          <image v-if="rightBtnText" :src="`${imgUrl}toRight2.png`" mode="aspectFit"></image>
        </view>
      </view>
      <view class="number_keyboard_value">
        <view>
          <text>{{ value || 0 }}</text>
          <text class="unit">kg</text>
        </view>
      </view>
      <view class="number_keyboard_content">
        <view
          class="number_keyboard_item"
          @click="handleClickKeyboard(item)"
          v-for="(item, index) in numberKeyboardOptions"
          :key="index"
        >
          <text>{{ item.value }}</text>
        </view>
        <view class="number_keyboard_item" @touchstart.stop="backspaceClick" @touchend="clearTimer">
